...
class TestWout(ConceptorTestBase):
    ...
    def test_reproduction(self):
        # TODO: Use Wout with the recorded states to recreate the original signal

        washout = 500

        P = np.asarray(self.C.sig[0])[washout:]

        # For each other signal
        for i in range(1, len(self.C.sig)):
            # Add the signal without the washout period to P
            P = np.concatenate((P, np.asarray(self.C.sig[i])[washout:]), axis=0)

        reg_sig = np.zeros(1)

        for i in range(len(self.C.state_response)):
            temp_reg_sig = np.zeros(1)

            for j in range(len(self.C.state_response[i])):
                res = self.C.Wout@ self.C.state_response[i][:, j]
                temp_reg_sig = np.concatenate((temp_reg_sig, res), axis=0)

            reg_sig = np.concatenate((reg_sig, temp_reg_sig[washout + 1:]), axis=0)

        self.assertTrue(self.errors(reg_sig[1:], P))
